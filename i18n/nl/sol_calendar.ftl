# Sol Calendar - Nederlandse vertaling

# Application
app-title = Sol Agenda
app-description = Een agendatoepassing voor het COSMIC-bureaublad

# Menu items
menu-file = Bestand
menu-edit = Bewerken
menu-view = Weergave
menu-new-event = Nieuwe afspraak...
menu-settings = Instellingen...
menu-month-view = Maandweergave
menu-week-view = Weekweergave
menu-day-view = Dagweergave
menu-show-week-numbers = Weeknummers tonen
menu-about = Over Sol Agenda
menu-quit = Afsluiten

# Views
view-month = Maand
view-week = Week
view-day = Dag

# Navigation
nav-today = Vandaag
nav-previous = Vorige
nav-next = Volgende

# Sidebar
sidebar-calendars = Agenda's
sidebar-mini-calendar = Mini-agenda

# Week number
week-abbr = Wk

# Calendar actions
calendar-add = Agenda toevoegen
calendar-edit = Bewerken
calendar-delete = Verwijderen
calendar-toggle = Zichtbaarheid schakelen

# Days of week (abbreviated)
day-sun = Zo
day-mon = Ma
day-tue = Di
day-wed = Wo
day-thu = Do
day-fri = Vr
day-sat = Za

# Days of week (full)
day-sunday = Zondag
day-monday = Maandag
day-tuesday = Dinsdag
day-wednesday = Woensdag
day-thursday = Donderdag
day-friday = Vrijdag
day-saturday = Zaterdag

# Months
month-january = Januari
month-february = Februari
month-march = Maart
month-april = April
month-may = Mei
month-june = Juni
month-july = Juli
month-august = Augustus
month-september = September
month-october = Oktober
month-november = November
month-december = December

# Events
event-new = Nieuwe afspraak
event-edit = Afspraak bewerken
event-delete = Afspraak verwijderen
event-title = Titel
event-location = Locatie
event-description = Beschrijving
event-start = Start
event-end = Einde
event-all-day = Hele dag

# Buttons
button-save = Opslaan
button-cancel = Annuleren
button-delete = Verwijderen
button-close = Sluiten

# Search
search-placeholder = Afspraken zoeken...
search-no-results = Geen afspraken gevonden

# Settings
settings-general = Algemeen
settings-appearance = Uiterlijk
settings-calendars = Agenda's
settings-advanced = Geavanceerd

# About dialog
about-version = Versie
about-repository = Repository
about-license = Licentie
about-author = Auteur
