# Sol Calendar - Tłumaczenie polskie

# Application
app-title = Kalendarz Sol
app-description = Aplikacja kalendarza dla środowiska COSMIC

# Menu items
menu-file = Plik
menu-edit = Edycja
menu-view = Widok
menu-new-event = Nowe wydarzenie...
menu-settings = Ustawienia...
menu-month-view = Widok miesiąca
menu-week-view = Widok tygodnia
menu-day-view = Widok dnia
menu-show-week-numbers = Pokaż numery tygodni
menu-about = O Kalendarzu Sol
menu-quit = Zakończ

# Views
view-month = Miesiąc
view-week = Tydzień
view-day = Dzień

# Navigation
nav-today = Dziś
nav-previous = Poprzedni
nav-next = Następny

# Sidebar
sidebar-calendars = Kalendarze
sidebar-mini-calendar = Mini kalendarz

# Week number
week-abbr = Tydz

# Calendar actions
calendar-add = Dodaj kalendarz
calendar-edit = Edytuj
calendar-delete = Usuń
calendar-toggle = Przełącz widoczność

# Days of week (abbreviated)
day-sun = Niedz
day-mon = Pon
day-tue = Wt
day-wed = Śr
day-thu = Czw
day-fri = Pt
day-sat = Sob

# Days of week (full)
day-sunday = Niedziela
day-monday = Poniedziałek
day-tuesday = Wtorek
day-wednesday = Środa
day-thursday = Czwartek
day-friday = Piątek
day-saturday = Sobota

# Months
month-january = Styczeń
month-february = Luty
month-march = Marzec
month-april = Kwiecień
month-may = Maj
month-june = Czerwiec
month-july = Lipiec
month-august = Sierpień
month-september = Wrzesień
month-october = Październik
month-november = Listopad
month-december = Grudzień

# Events
event-new = Nowe wydarzenie
event-edit = Edytuj wydarzenie
event-delete = Usuń wydarzenie
event-title = Tytuł
event-location = Lokalizacja
event-description = Opis
event-start = Początek
event-end = Koniec
event-all-day = Cały dzień

# Buttons
button-save = Zapisz
button-cancel = Anuluj
button-delete = Usuń
button-close = Zamknij

# Search
search-placeholder = Szukaj wydarzeń...
search-no-results = Nie znaleziono wydarzeń

# Settings
settings-general = Ogólne
settings-appearance = Wygląd
settings-calendars = Kalendarze
settings-advanced = Zaawansowane

# About dialog
about-version = Wersja
about-repository = Repozytorium
about-license = Licencja
about-author = Autor
